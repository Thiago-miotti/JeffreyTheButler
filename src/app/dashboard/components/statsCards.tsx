...
export default function StockStatusCard({
  ...
  return (
    <div
      className={`card w-full bg-base-100 shadow-lg border-l-4 ${style.border} transition-all duration-300`}
    >
      <div className="card-body p-4 3xl:p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className={`text-6xl font-bold ${style.text}`}>{count}</div>
        </div>
        ...
        <div className="space-y-1">
          ...
          <p className="text-md text-base-content/70 font-default">{description}</p>
        </div>
      </div>
    </div>
  )
