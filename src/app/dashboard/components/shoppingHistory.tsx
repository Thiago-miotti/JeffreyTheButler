      ...
      <div className="overflow-auto flex flex-col gap-4 max-h-62">
        {Object.entries(grouped).map(([month, buys]) => (
          <div key={month} className="flex flex-col gap-2">
            <p className="text-neutral/50 font-semibold capitalize">{month}</p>

            {buys.map((buy) => (
              <div
                key={buy.id}
                className="w-full bg-transparent border border-base-300 px-4 py-1 rounded-lg flex items-center justify-between"
              >
                <div className="flex flex-col gap-1">
                  <h1 className="text-xl text-neutral font-semibold">
                    {buy.supermarketName}
                  </h1>
                  <div className="flex gap-4">
                    <p className="text-black/60 font-semibold">
                      {formatISOToBRL(buy.purchaseDate.toString())}
                    </p>
                    <p className="text-black/60 font-semibold">
                      {FormatToBRL(buy.puchaseTotal)}
                    </p>
                  </div>
                </div>
